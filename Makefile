...
#
# Requires: google-chrome (headless), ImageMagick 7 (`magick`).
...
og: ## Re-render og.png from tools/og.html
	$(CHROME) --headless=new --disable-gpu --hide-scrollbars \
		...
		--screenshot=/tmp/bei-og-raw.png --window-size=1200,630 tools/og.html
	$(MAGICK) /tmp/bei-og-raw.png -strip -colors 16 PNG8:og.png
	@rm -f /tmp/bei-og-raw.png
	@echo "wrote og.png"

.PHONY: favicon
favicon: ## Re-render favicon.ico and apple-touch-icon.png from the SVG sources
	$(MAGICK) -background black favicon.svg -resize 180x180 \
		-alpha off -colorspace sRGB -type TrueColor -strip apple-touch-icon.png
	$(MAGICK) \
		\( -background black favicon.svg -resize 48x48 \) \
		\( -background black favicon.svg -resize 32x32 \) \
		\( -background black tools/favicon-16.svg -resize 16x16 \) \
